...
function StatusBadge({ imported, role }: { imported: boolean; role?: any }) {
  if (!imported) return <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: '#f3f4f6', color: '#6b7280', border: '1px solid #e5e7eb', fontWeight: 600 }}>Available</span>;
  const color = role?.status === 'open' ? '#16a34a' : '#2563eb';
  const label = role?.status === 'open' ? 'Live' : role?.status === 'draft' ? 'Draft' : role?.status || 'Imported';
  return <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: color + '15', color, border: '1px solid ' + color + '30', fontWeight: 600 }}>{label}</span>;
}
  ...
  return (
    <div style={{ paddingTop: 8 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-bright)', letterSpacing: '-0.4px', marginBottom: 4 }}>Job Bank</h1>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {selected.size > 0 && (
            <button onClick={handleBulkImport} disabled={bulkImporting}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 7, background: 'var(--blue)', color: '#fff', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', opacity: bulkImporting ? 0.7 : 1 }}>
              <IconDownload size={13} /> Import {selected.size} selected
            </button>
          )}
          <button onClick={selectAll} style={{ padding: '7px 12px', borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 11, color: 'var(--text-mid)', cursor: 'pointer' }}>
            Select all available
          </button>
          <button onClick={handleRefresh} disabled={refreshing}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 12, color: 'var(--text-mid)', cursor: 'pointer', opacity: refreshing ? 0.7 : 1 }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Total in ATS', value: stats.total, color: 'var(--text-bright)' },
            { label: 'Available to import', value: stats.available, color: '#2563eb' },
            { label: 'Imported to Taltas', value: stats.imported, color: '#16a34a' },
            { label: 'Connected ATS', value: stats.byProvider.length, color: 'var(--text-mid)' },
          ].map(s => (
            <div key={s.label} style={{ background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 10, padding: '14px 18px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search jobs, departments, locations..."
            style={{ width: '100%', paddingLeft: 30, paddingRight: 10, paddingTop: 8, paddingBottom: 8, borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 12, color: 'var(--text-bright)', outline: 'none' }} />
        </div>
        <select value={providerFilter} onChange={e => setProviderFilter(e.target.value)}
          style={{ padding: '8px 12px', borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 12, color: 'var(--text-mid)', cursor: 'pointer' }}>
        ...
        {(search || providerFilter || importedFilter) && (
          <button onClick={() => { setSearch(''); setProviderFilter(''); setImportedFilter(''); }}
            style={{ padding: '7px 10px', borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', cursor: 'pointer', color: 'var(--muted)' }}>
            <IconX size={13} />
          </button>
        )}
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border2)' }}>
              ...
              {['Job Title', 'Department', 'Location', 'ATS Source', 'Salary', 'Status', ''].map(h => (
                <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr key={job.id} style={{ borderBottom: i < jobs.length - 1 ? '1px solid var(--border)' : 'none', background: selected.has(job.id) ? 'var(--blue-bg)' : 'transparent', transition: 'background 0.1s' }}
                onMouseEnter={e => { if (!selected.has(job.id)) (e.currentTarget as HTMLElement).style.background = 'var(--surface2)'; }}
                ...
                <td style={{ padding: '12px 14px' }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-bright)', cursor: 'pointer' }} onClick={() => setDetailJob(job)}>{job.title}</div>
                  <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 2 }}>{job.integrationName}</div>
                </td>
                <td style={{ padding: '12px 14px', fontSize: 12, color: 'var(--text-mid)' }}>{job.department || '—'}</td>
                <td style={{ padding: '12px 14px', fontSize: 12, color: 'var(--text-mid)' }}>{job.location || '—'}</td>
                <td style={{ padding: '12px 14px' }}><ProviderBadge provider={job.provider} /></td>
                <td style={{ padding: '12px 14px', fontSize: 12, color: 'var(--text-mid)' }}>{formatSalary(job.salaryMin, job.salaryMax) || '—'}</td>
                ...
                <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                  {job.imported ? (
                    ...
                    <button onClick={() => setDetailJob(job)} disabled={importing === job.id}
                      style={{ padding: '5px 12px', borderRadius: 6, background: 'var(--blue)', color: '#fff', fontSize: 11, fontWeight: 600, border: 'none', cursor: 'pointer', opacity: importing === job.id ? 0.7 : 1 }}>
          ...
          <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'center', gap: 6 }}>
            {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => fetchJobs(p)}
                style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid var(--border2)', background: p === page ? 'var(--blue)' : 'var(--surface)', color: p === page ? '#fff' : 'var(--text-mid)', fontSize: 11, cursor: 'pointer', fontWeight: p === page ? 700 : 400 }}>
                {p}
              </button>
            ))}
          </div>
        ...
        <Modal onClose={() => setDetailJob(null)}>
          <div style={{ padding: 28, minWidth: 480, maxWidth: 560 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-bright)', marginBottom: 4 }}>{detailJob.title}</div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {detailJob.description && (
              <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: 14, marginBottom: 18, fontSize: 12, color: 'var(--text-mid)', lineHeight: 1.6, maxHeight: 140, overflowY: 'auto' }}>
            ...
            {formatSalary(detailJob.salaryMin, detailJob.salaryMax) && (
              <div style={{ marginBottom: 16, fontSize: 12, color: 'var(--text-mid)' }}>
                <span style={{ fontWeight: 600 }}>Salary: </span>{formatSalary(detailJob.salaryMin, detailJob.salaryMax)}
              </div>
            )}

            <div style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-mid)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Import options</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  ...
                  <select value={importOpts.urgency} onChange={e => setImportOpts(o => ({ ...o, urgency: e.target.value }))}
                    style={{ width: '100%', padding: '8px 10px', borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 12, color: 'var(--text-bright)' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: 'var(--text-mid)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setDetailJob(null)} style={{ padding: '8px 16px', borderRadius: 7, border: '1px solid var(--border2)', background: 'var(--surface)', fontSize: 12, color: 'var(--text-mid)', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => handleImport(detailJob)} disabled={importing === detailJob.id}
                style={{ padding: '8px 20px', borderRadius: 7, background: 'var(--blue)', color: '#fff', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', opacity: importing === detailJob.id ? 0.7 : 1 }}>
